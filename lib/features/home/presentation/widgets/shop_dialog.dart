                      ...
                      children: [
                        ...
                        if (state.currentWorldIndex == 1) ...[
                          ...
                          _buildShopItem(
                            ...
                            icon: Icons.opacity,
                            title: 'WATER BUCKET',
                            description: 'Refill the oasis timer',
                            count: state.itemWaterBucket,
                            puzzleCost: 25,
                            moneyCost: 0.59,
                            onBuyPuzzles: () => context.read<HomeBloc>().add(const BuyItem('water_bucket', 25)),
                            onBuyMoney: () => context.read<HomeBloc>().add(const BuyItem('water_bucket', 0, isRealMoney: true)),
                          ),
                          const SizedBox(height: 10),
                          _buildShopItem(
                            context,
                            icon: Icons.build,
                            title: 'GOLDEN WRENCH',
                            description: 'Fix one pipe correctly',
                            count: state.itemGoldenWrench,
                            puzzleCost: 40,
                            moneyCost: 0.79,
                            onBuyPuzzles: () => context.read<HomeBloc>().add(const BuyItem('golden_wrench', 40)),
                            onBuyMoney: () => context.read<HomeBloc>().add(const BuyItem('golden_wrench', 0, isRealMoney: true)),
                          ),
                          const SizedBox(height: 10),
                          _buildShopItem(
                            context,
                            icon: Icons.handyman,
                            title: 'SAND SHOVEL',
                            description: 'Clear a sandstorm block',
                            count: state.itemSandShovel,
                            puzzleCost: 60,
                            moneyCost: 1.29,
